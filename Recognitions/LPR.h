...
private:
	...
	{
		sort(res.begin(), res.end(), [](const std::pair<string, float>& a, 
			const pair<string, float>& b) { return a.second > b.second; });
	...
	{
		if (std::isdigit(str[0]) || std::isdigit(str[1]) || !std::isdigit(str[2]) || !std::isdigit(str[5]) 
	...
	Rect Detect_LPR(Net net, Mat frame1, Mat frame, float w, float h)
	{
		...
		for (int i = 0; i < detectionMat.rows; i++)
		{
			...
			if (confidence > 0.35)
			{
				...
				if (0 <= t.x && 0 <= t.width && t.x + t.width <= frame1.cols && 0 <= t.y 
					&& 0 <= t.height && t.y + t.height <= frame1.rows)
					return t;
			}
			else
				continue;
